...
It basically has the same sound as TSynth, but offers it in a smaller unit with more features that allow for further development. It was designed to be easier and probably cheaper than TSynth. The case is just a 3D printed tray with a PCB front panel. There is about the same amount of soldering, but without the problems of doldering the Teensy 4.1 and audio shield. Yes the cut-out for the audio socket is because I used the small PCM5102A PCBs that cost less than $2 and have the support components with them. This could have been soldered on the main PCB but it raises the cost and it's part of the DIY nature of Momentum.
